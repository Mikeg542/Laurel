		...
		foreach ( $options as $value ) {
			...
			case 'typography':
				...
				if ( $typography_options['weights'] ) {
					$font_weight = '<select class="of-typography of-typography-weight" name="' . esc_attr( $option_name . '[' . $value['id'] . '][weight]' ) . '" id="' . esc_attr( $value['id'] . '_weight' ) . '">';
					$weights = $typography_options['weights'];
					foreach ( $weights as $i=>$val ) {
						$font_weight .= '<option value="' . esc_attr( $i ) . '" ' . selected( $typography_stored['weight'], $i, false ) . '>' . esc_html( $val ) . '</option>';
					}
					$font_weight .= '</select>';
				}
